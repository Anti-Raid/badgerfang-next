			...
			<Link href={`/blogs/${blog.slug}`} className="flex flex-col h-full">
				<div className="relative h-full flex flex-col overflow-hidden rounded-2xl bg-card/40 backdrop-blur-md border border-white/5 group-hover:border-primary/30 transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-primary/10 shadow-lg">
					...
					<div className={`flex flex-col ${isFeatured ? 'md:flex-row h-full' : 'h-full'}`}>
						{/* Image Container */}
						<div className={`relative overflow-hidden ${isFeatured ? 'w-full md:w-3/5 h-64 md:h-auto aspect-video' : 'w-full h-56'}`}>
							{blog.slug ? (
								<Image
						...
						{/* Content Section */}
						<div className={`p-6 flex flex-col justify-between flex-grow ${isFeatured ? 'md:w-2/5 md:p-10' : ''}`}>
							<div className="space-y-4">
								<h3 className={`font-monster font-bold group-hover:text-primary transition-colors leading-tight ${isFeatured ? 'text-2xl md:text-3xl lg:text-4xl' : 'text-xl'}`}>
									{blog.title}
								</h3>
								<p className={`text-muted-foreground line-clamp-3 leading-relaxed ${isFeatured ? 'text-base lg:text-lg' : 'text-sm'}`}>
									{blog.description}
								</p>
							</div>

							<div className={`mt-8 flex items-center justify-between ${isFeatured ? 'border-t border-border/10 pt-8' : ''}`}>
								<div className="flex items-center gap-3">
									...
									<div className="flex flex-col">
										...
										<span className="text-xs text-muted-foreground">{format(new Date(blog.publishedAt), 'MMM d, yyyy')}</span>
									</div>
								</div>
