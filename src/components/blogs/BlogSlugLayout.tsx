      ...
      try {
        ...
        if (foundBlog) {
          ...
          if (foundBlog.tags && foundBlog.tags.length > 0) {
            const related = data
              .filter((b) => b.slug !== slug && b.tags?.some((tag) => foundBlog.tags?.includes(tag)))
              .slice(0, 2)
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          ...
          >
            <motion.span initial={{ x: 0 }} animate={{ x: 0 }} whileHover={{ x: -3 }} transition={{ duration: 0.2 }}>
          ...
          >
            <ReactMarkdown
              ...
              components={{
                ...
                p: ({ node, ...props }) => (
                  <p 
                    className="my-4 leading-relaxed text-base md:text-lg" 
                    {...props} 
                  />
                ),
                a: ({ node, ...props }) => (
                ...
                ul: ({ node, ...props }) => (
                  <ul 
                    className="my-4 ml-6 list-disc space-y-2 marker:text-primary/70" 
                    {...props} 
                  />
                ),
                ol: ({ node, ...props }) => (
                ...
                hr: ({ node, ...props }) => (
                  <hr 
                    className="my-8 border-border/60" 
                    {...props} 
                  />
                ),
                table: ({ node, ...props }) => (
                  <div className="overflow-x-auto my-8">
                    <table 
                      className="w-full border-collapse text-sm text-white" 
                      {...props} 
                    />
                  </div>
                ),
                ...
                td: ({ node, ...props }) => (
                  <td 
                    className="border border-border px-4 py-2 text-white" 
                    {...props} 
                  />
                ),
              }}
              ...
              {relatedBlogs.map((relatedBlog, index) => (
                ...
                >
                  <Link href={`/blogs/${relatedBlog.slug}`} className="block">
                    ...
                    <p className="text-muted-foreground text-sm line-clamp-2 mb-4">{relatedBlog.description}</p>
