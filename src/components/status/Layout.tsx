...
import { Shield, Server, Activity, Clock, Check, AlertTriangle, X, LayoutDashboard, BarChart3 } from "lucide-react"
...
export default function StatusPage() {
  ...
  const chartData = useMemo(() => {
    ...
    const guilds = Object.entries(data.shard_conns).map(([k, s]) => ({ name: `Shard ${k}`, guilds: s?.guilds ?? 0 }))
    const uptime = Object.entries(data.shard_conns).map(([k, s]) => ({ name: `Shard ${k}`, uptime: s?.uptime ?? 0 }))
    ...
    const status = Object.entries(counts).map(([label, value]) => ({ label, value, color: statusColor(label) }))
  ...
  const summary = useMemo(() => {
    ...
    const avgLatency = Math.round(
      Object.values(data.shard_conns).reduce((sum, s) => sum + (s?.real_latency ?? 0), 0) / Math.max(1, totalShards),
    )
      ...
      <header className="mb-8">
        <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-pretty text-muted-foreground">Real-time monitoring for all shards and services</p>
          </div>
          ...
          <div className="rounded-lg border border-border bg-card p-1.5">
            <nav aria-label="Tabs" className="flex items-center gap-1">
              {(["overview", "shards", "charts"] as TabKey[]).map((k) => {
                ...
                const Icon = k === "overview" ? LayoutDashboard : k === "shards" ? Server : BarChart3
                return (
                  <button
                    key={k}
                    onClick={() => setTab(k)}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              <span
                ...
                style={{ backgroundColor: summary.health > 90 ? "hsl(142,76%,36%)" : "hsl(38,92%,50%)" }}
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 rounded-xl border border-border/60 bg-card p-6">
            ...
            <div className="h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData.latency.slice(0, 12)} margin={{ top: 8, right: 16, left: 0, bottom: 40 }}>
                  ...
                  />
                  <Bar dataKey="latency" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} maxBarSize={28} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2 text-sm">
                <span className="inline-block h-3 w-3 rounded-full" style={{ backgroundColor: "hsl(142,76%,36%)" }} />
                <span>Ready</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span className="inline-block h-3 w-3 rounded-full" style={{ backgroundColor: "hsl(38,92%,50%)" }} />
                <span>Marked</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <span
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-border/60 bg-card p-6">
            ...
            <div className="h-[360px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData.latency} margin={{ top: 8, right: 16, left: 0, bottom: 40 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" opacity={0.3} />
                  <XAxis
                    dataKey="name"
                    angle={-35}
          ...
          <div className="rounded-xl border border-border/60 bg-card p-6">
            ...
            <div className="h-[360px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData.guilds} margin={{ top: 8, right: 16, left: 0, bottom: 40 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" opacity={0.3} />
                  <XAxis
                    dataKey="name"
                    angle={-35}
                  ...
                  />
                  <Bar dataKey="guilds" fill="hsl(var(--chart-2))" radius={[4, 4, 0, 0]} maxBarSize={28} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="h-[380px]">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Bar dataKey="uptime" fill="hsl(142,76%,36%)" radius={[0, 4, 4, 0]} maxBarSize={18} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border/60 bg-card p-6">
            ...
            <div className="h-[360px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData.latency} margin={{ top: 8, right: 16, left: 0, bottom: 40 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" opacity={0.3} />
                  <XAxis
                    dataKey="name"
                    angle={-35}
