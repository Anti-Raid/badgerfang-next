...
export default function FlowNodeExplorer() {
	...
	return (
		<div className="w-30 h-full flex flex-col">
			...
			<div className="flex-auto mr-1">
				<div className="space-y-3 pl-2 pr-1 pb-5">
					{sections.map((section, i) => (
						<div key={i}>
							<div className="text-foreground font-medium mb-2 px-1">{section.title}</div>
			...
			<div className="flex-none px-4 py-1 text-sm text-muted-foreground">Current nodes.</div>
		</div>
	);
}
	...
	return (
		<div className="flex space-x-3 text-lg mb-3 px-5 justify-between text-muted-foreground border-b-2 border-dark-5">
			<div onClick={() => setCategory('action')} className="cursor-pointer">
				...
				<div className={clsx('h-1 rounded', category === 'action' && 'bg-primary')}></div>
			</div>
			<div onClick={() => setCategory('control_flow')} className="cursor-pointer">
				<div
					className={clsx(
						'pb-2 px-3 hover:text-foreground',
				...
				<div className={clsx('h-1 rounde', category === 'control_flow' && 'bg-primary')}></div>
			</div>
		</div>
	);
}
		...
		>
			<div className="flex items-start space-x-3">
				<div className="overflow-hidden">
					...
					<div className="text-sm text-muted-foreground">{values.defaultDescription}</div>
				</div>
			</div>
		</div>
	);
