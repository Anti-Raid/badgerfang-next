...
const FlowExpandedModal: React.FC<FlowExpandedProps> = ({ nodeProps, children, title, onDone }) => {
	...
	const modal = isExpanded ? (
		<div
			...
			className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in-0 duration-300"
		>
			<div
				onClick={(e) => e.stopPropagation()}
				className="relative w-full max-w-4xl max-h-[90vh] overflow-hidden animate-in fade-in-0 zoom-in-95 slide-in-from-bottom-4 duration-200"
			>
				...
				<div className="relative bg-card/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-border/50 overflow-hidden">
					...
					{/* Content */}
					<div className="relative px-6 py-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
						<div className="prose prose-sm max-w-none dark:prose-invert">{children}</div>
					</div>

					{/* Footer */}
					<div className="relative px-6 py-4 border-t border-border/50 bg-muted/30 backdrop-blur-sm">
