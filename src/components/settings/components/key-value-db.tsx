      ...
      <div className="mt-6">
        ...
        {keyValuePairs.length > 0 ? (
          keyValuePairs.map((pair) => (
            <div key={pair.key} className="bg-background border border-primary border-opacity-20 rounded-md p-4 mb-4">
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-medium text-foreground">Key: {pair.key}</p>
                  <p className="text-muted-foreground">Value: {pair.value}</p>
                </div>
                <Primary Title="Delete" onClick={() => handleDeleteKeyValue(pair.key)} icon={FaTrash} />
              </div>
            </div>
          ))
        ) : (
