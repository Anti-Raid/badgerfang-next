...
const Select: React.FC<SelectProps> = ({ value, onChange, options, placeholder = "Select...", className = "" }) => {
              ...
              {options.map((option) => (
                <div
                  ...
                  className={`px-3 py-2.5 cursor-pointer transition-colors duration-150 ${
                    option.value === value ? "bg-primary/10 text-primary" : "hover:bg-secondary text-foreground"
                  }`}
                  onClick={() => {
...
export default function CommandInterface() {
  ...
  const CommandCard: React.FC<{ command: CommandWithModule; index: number }> = ({ command, index }) => {
    ...
    return (
      ...
      >
        <div className="p-5">
          ...
          {command.description && <p className="text-muted-foreground mb-4 line-clamp-2">{command.description}</p>}
          ...
          <div className="space-y-3">
            {command.subcommands && command.subcommands.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {command.subcommands.slice(0, expanded ? command.subcommands.length : 3).map((subCmd) => (
            ...
            {command.arguments.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {command.arguments.slice(0, expanded ? command.arguments.length : 3).map((arg) => (
            ...
            {botState?.command_permissions && botState.command_permissions[command.qualified_name || command.name] && (
              <div className="space-y-2">
                <p className="text-sm font-medium flex items-center gap-1.5">
                  <ArrowRight className="h-3.5 w-3.5 text-primary" />
                ...
                <div className="flex flex-wrap gap-1.5">
                  {botState.command_permissions[command.qualified_name || command.name]
                    .slice(
                      0,
                      expanded ? botState.command_permissions[command.qualified_name || command.name].length : 2,
                    )
                    .map((perm, idx) => (
                  ...
                  {!expanded && botState.command_permissions[command.qualified_name || command.name].length > 2 && (
                    <Badge variant="outline" onClick={() => setExpanded(true)}>
                      +{botState.command_permissions[command.qualified_name || command.name].length - 2} more
                    </Badge>
                  )}
                </div>
              </div>
                  ...
                  <ul className="space-y-3">
                    {command.subcommands.map((subCmd) => (
                      <li key={subCmd.name} className="text-sm bg-secondary/30 p-3 rounded-lg">
                        ...
                        {subCmd.description && <p className="text-muted-foreground mt-1">{subCmd.description}</p>}
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <ul className="space-y-3">
                    {command.arguments.map((arg) => (
                      <li key={arg.name} className="text-sm bg-secondary/30 p-3 rounded-lg">
                        ...
                        {arg.description && <p className="text-muted-foreground mt-1">{arg.description}</p>}
                ...
                botState.command_permissions[command.qualified_name || command.name] && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 bg-secondary/30 p-3 rounded-lg">
                      {botState.command_permissions[command.qualified_name || command.name].map((perm, idx) => (
                        <Badge key={idx} variant="secondary">
                          {permissionNames[perm] || perm}
                        </Badge>
                      ))}
  ...
  const CommandListItem: React.FC<{ command: CommandWithModule; index: number }> = ({ command, index }) => {
    const [expanded, setExpanded] = useState(false)
        ...
        <div className="p-5">
          <div className="flex justify-between items-center">
            <div className="flex-1">
              ...
              {command.description && <p className="text-muted-foreground mt-1 line-clamp-1">{command.description}</p>}
            </div>
            <button
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`p-2 rounded-lg transition-colors ${
                        activeView === "grid" ? "bg-primary text-white" : "bg-secondary/50 hover:bg-secondary"
                      }`}
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg transition-colors ${
                        activeView === "list" ? "bg-primary text-white" : "bg-secondary/50 hover:bg-secondary"
                      }`}
