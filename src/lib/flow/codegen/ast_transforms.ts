...
export class ASTPreludeApply extends BaseTransform {
    transform() {
            ...
            case IPreludeTypeEnum.Command:
                ...
                */
                ...
                for (let arg of this.ast.prelude.data.arguments) {
                    ...
                    if (arg.description) {
                        registerFuncStart.push(`\t\t\t:setDescription("${arg.description.replaceAll('"', '\"')}")`)
                    }
                    if (arg.required) {
                ...
                if (this.ast.dependencies['framework']) {
                    this.ast.warnings.push("framework is defined in dependencies but is also used by prelude")
                }
        ...
        if (this.ast.prelude.type !== IPreludeTypeEnum.Applied) {
            this.ast.errors.push("Internal error: Prelude was not applied correctly, please report this issue");
        }
    }
}