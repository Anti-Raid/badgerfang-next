...
export class CodeGenIRGenerator {
     ...
     */
    private visitNode(currentIr: CodeGenIR, node: Node<NodeData>): VisitResult {
     ...
     */
    private visitNodeAndChildren(currentIr: CodeGenIR, nodeId: string): INode[] {
    ...
    private visitSetVariable(node: Visit<VariableSetNode>): VisitResult {
        ...
        if (children.length == 1) {
            ...
            this.pushWarning(node.currentIr, `VariableSetNode ${node.nodeId} has multiple children, only the first will be considered.`);
            nextNode = children[0].id; // Take the first child
        }
    ...
    private visitCustomCode(node: Visit<CustomCodeNode>): VisitResult {
        ...
        if (children.length == 1) {
            ...
            this.pushWarning(node.currentIr, `CustomCodeNode ${node.nodeId} has multiple children, only the first will be considered.`);
            nextNode = children[0].id; // Take the first child
        }
    ...
    private visitIfCondition(node: Visit<IfConditionNode>): VisitResult {
        ...
        for (const child of children) {
            ...
            switch (childData.type) {
                ...
                case NodeTypeEnum.ElseCondition:
                    if (elseNodeId) {
                        this.pushWarning(node.currentIr, `IfCondition ${node.nodeId} has multiple Else nodes, only the first will be considered.`);
                    } else {
                        elseNodeId = child.id;
                    }
                    break;
                case NodeTypeEnum.EndCondition:
                    if (endNodeId) {
                        this.pushWarning(node.currentIr, `IfCondition ${node.nodeId} has multiple End nodes, only the first will be considered.`);
                    } else {
                        endNodeId = child.id;
                    }
                    break;
                default:
                    if (bodyStart) {
                        this.pushWarning(node.currentIr, `IfCondition ${node.nodeId} has multiple Block nodes, only the first will be considered.`);
                    } else {
                        bodyStart = child.id;
                    }
                    break;
        ...
        let bodyNodes: INode[] = [];
        if (bodyStart) {
            bodyNodes = this.visitNodeAndChildren(node.currentIr, bodyStart);
        }
        ...
        for (const elseifId of elseifNodeIds) {
            ...
            if (elseifChildren.length !== 1) {
                this.pushWarning(node.currentIr, `ElseIfCondition ${elseifId} has multiple outgoing connections, only the first will be considered.`);
            }
            ...
            elseIfs.push({
                ...
                body: this.visitNodeAndChildren(node.currentIr, elseifChildren[0].id),
            });
        }
        ...
        if (elseNodeId) {
            ...
            if (elseChildren.length !== 1) {
                this.pushWarning(node.currentIr, `ElseCondition ${elseNodeId} has multiple outgoing connections, only the first will be considered.`);
            }
            elseBlock = this.visitNodeAndChildren(node.currentIr, elseChildren[0].id);
        }
        ...
        if (endChildren.length > 1) {
            this.pushWarning(node.currentIr, `EndCondition ${endNodeId} has multiple outgoing connections, only the first will be considered.`);
        }
    ...
    private visitForLoop(node: Visit<ForLoopNode>): VisitResult {
        ...
        for (const child of children) {
            ...
            switch (childData.type) {
                case NodeTypeEnum.EndCondition:
                    if (endNodeId) {
                        this.pushWarning(node.currentIr, `ForLoop ${node.nodeId} has multiple End nodes, only the first will be considered.`);
                    } else {
                        endNodeId = child.id;
                    }
                    break;
                default:
                    if (bodyStart) {
                        this.pushWarning(node.currentIr, `ForLoop ${node.nodeId} has multiple Block nodes, only the first will be considered.`);
                    } else {
                        bodyStart = child.id;
                    }
                    break;
