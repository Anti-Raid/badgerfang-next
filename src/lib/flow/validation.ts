...
export const baseCommandNodeSchema = type({
	...
	if (data.name) {
		...
		if (split.length > 3) {
			throw new Error('Command name can have at most 3 words (base command, subcommand, subcommand group)');
		}
		
		for (const word of split) {
			commandNameRegex.lastIndex = 0; // Reset regex
			if (!commandNameRegex.test(word)) {
				throw new Error(`Each part of a command name must be only lowercase alphanumeric characters and underscores: ${word}`);
			}
		}
	}
...
export const commandArgumentNodeSchema = type({
	...
	if (!data.command_argument_name || data.command_argument_name.length < 1 || data.command_argument_name.length > 32) {
