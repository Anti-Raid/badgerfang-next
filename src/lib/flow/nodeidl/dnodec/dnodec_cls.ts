...
export class ModuleImports {
    ...
    addImport(to: string, model: DModel) {
        if (!/^[A-Z][a-zA-Z0-9_]*$/.test(to)) {
            throw new Error(`Import destinations must be alphanumeric or underscore and must start with a uppercase letter. Invalid id: ${to}`);
        }
...
export class DField {
    ...
    constructor(imports: ModuleImports, type: DFieldType, shortname: string, id: string, description: string) {
    ...
    private gen(): nodeidl.Field {
        if (this.type.type === "scalar") {
            ...
            let innerElem = new DField(this.imports, this.type.elementType, this.shortname, this.id, this.description);
            return {
                type: "array",
...
export class DModel {
    ...
    constructor(shortname: string, id: string, description: string, imports: ModuleImports, fields: DField[], code: string) {
        this.shortname = shortname;
        this.id = id;
        this.description = description;
        this.imports = imports;
    ...
    static fromJSON(json: any, importStorage: ImportStorage, filename: string): DModel {
        if (typeof json !== "object" || json === null) {
            throw new Error(`DModel JSON must be an object when processing file ${filename}. with current obj: ${JSON.stringify(json)}`);
        }
...
export class FlowUI {
    ...
    static fromJSON(json: any, importStorage: ImportStorage, imports: ModuleImports, filename: string): FlowUI {
        if (typeof json !== "object" || json === null) {
            throw new Error("FlowUI JSON must be an object.");
        }

        if (typeof json.handles !== "object" || json.handles === null || !Array.isArray(json.handles.allow)) {
...
export class DNode {
    ...
    constructor(shortname: string, id: string, description: string, imports: ModuleImports, code: string, flowui: FlowUI) {
        this.shortname = shortname;
        this.id = id;
        this.description = description;
        this.imports = imports;
