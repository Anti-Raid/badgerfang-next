...
interface WasmExports {
  cwrap: (fn: string, returnType: string, argTypes: string[]) => (code: string, args: string) => number;
  stackSave: () => number;
...
const getModule = async () => {
    ...
    try {
        ...
        module = { module: wasmModule, cwrapped: wasmModule.cwrap('luau_template', 'number', ['string', 'string']) };
        return module;
    } catch (error) {
        throw error;
    }
...
self.onmessage = async (event) => {
    ...
    try {
        ...
        self.postMessage({id, data: { status: LuauTemplateResultCode.ErrorFatal, message: error?.toString() || 'Unknown error' }});
    }
}
